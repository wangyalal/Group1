...
class _CellEditor:
    ...
    def __init__(self, tree: tb.Treeview, item_id: str, col_index: int,
        ...
        if col_name == "Type":
            ...
            # Pick category list based on current type value
            values_col = tree["columns"][3]  # Type column index
...
class LedgerFrame(tb.Frame):
    ...
    def _save_row(self, item_id, values):
        ...
        tx_items  = all_items[:-1]  # exclude net row
        idx       = list(tx_items).index(item_id)
        raw       = self._raw_rows[idx]

        tx_id = _get_transaction_id(
            tx_date=raw[0],
            description=raw[1],
            amount=raw[3],
            tx_type=raw[4],
